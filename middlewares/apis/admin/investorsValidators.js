...
exports.withdrawValidator = [
	...
	(req, res, next) => {
		...
		models.User.findByPk(investorId, {
			...
			req.investor = investor;
			aggregationsHelper.cashAvailableToWithdrawalInvestor(investorId).then(availableToWithdraw => {
				if (availableToWithdraw >= withdrawAmount) {
					next();
				}
				else {
					res.status(404).json({ message: "You don't have enough money to withdraw." })
				}
			});
		}).catch(error => {
			console.log(error);
